...
def orden_trabajo(dic, path=''):
    ...
    for otro in otros:
        ...
        otro_par.drawOn(canvas, 50, length - (220 + count))

        index = otros.index(otro)
        precio = otros_precios[index]
        par = Paragraph(f'<b>{precio}</b>', style=paragraph_style)
        par.wrap(width, length)
        par.drawOn(canvas, 450, length - (220 + count))
        count += 20

    paragraph_style = ParagraphStyle('style', fontSize=12, leading=10, wordWrap='CJK', font='Calibri',
                                     spaceShrinkage=0.5)
    ...
    unitario.drawOn(canvas, 350, length - (230 + count))
    count += 20

    total.wrap(width, length)
    total.drawOn(canvas, 350, length - (230 + count))
    ...
    count -= 20
    p_unitario_total_par.drawOn(canvas, 450, length - (230 + count))
    count += 20
    p_total_par.drawOn(canvas, 450, length - (230 + count))
